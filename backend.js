...
app.get('/search', async (req, res) => {
  // Accept key from env var OR from query param (sent by frontend)
  const apiKey = process.env.JSEARCH_API_KEY || req.query.rapidapi_key;
  if (!apiKey) return res.status(500).json({ error: 'No API key available' });
  try {
    // Strip rapidapi_key from forwarded params
    const { rapidapi_key, ...forwardParams } = req.query;
    const params = new URLSearchParams(forwardParams).toString();
